...
def get_guest_token():
    try:
        ...
        logging.error(f"Failed to get Guest Token: {e}")
    return None
...
def probe_money_api():
            ...
            if r.status_code == 200 and "<html" not in r.text[:50]:
                ...
                findings.append(finding)
                 ...
                 try:
                     ...
                     logging.warning(f"Could not parse error response from {url}")
        ...
        try:
            ...
            if r.status_code == 200 and "<html" not in r.text[:50]:
                ...
                finding = Finding(
                    ...
                    target=url,
                    confidence=0.8,
                    metadata={"method": "POST", "payload": payload}
                )
                findings.append(finding)
        except Exception as e:
            logging.error(f"POST Error for {url}: {e}")
    ...
    for p in params:
        ...
        try:
            ...
            logging.warning(f"Fuzzing failed for {full_url}: {e}")

    with open("money_leak.json", "w") as f:
        json.dump(findings, f, indent=2, cls=EnhancedJSONEncoder)
